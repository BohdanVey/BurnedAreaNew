...
def write_images(best_images, worst_images, writer):
    x = 0
    for img in best_images:
        x += 1
        img[0] = np.reshape(img[0].cpu(), (-1, img[0].shape[2], img[0].shape[3]))
        img[1] = np.reshape(img[1].cpu(), (-1, img[1].shape[2], img[1].shape[3]))
        writer.add_image(f'Best image {x} train', img[0])
        writer.add_image(f'Best image {x} real', img[1])
    x = 0
    for img in worst_images:
        ...
        writer.add_image(f'Worst image {x} train', img[0])
        writer.add_image(f'Worst image {x} real', img[1])
...
def val(model, val_loader, loss_f, metric_fns, device):
    ...
    with torch.no_grad():
        for data, target in tqdm(val_loader):
            ...
            output = model(data)
            loss = loss_f(output, target)
            best_imgs.append([(output * 255).int(), target.int() * 90, float(loss)])
            best_imgs = sorted(best_imgs, key=lambda x: x[2])
            best_imgs = best_imgs[:5]
            worst_imgs.append([(output * 255).int(), target.int() * 90, float(loss)])
            worst_imgs = sorted(worst_imgs, key=lambda x: -x[2])
            worst_imgs = worst_imgs[:5]
            batch_size = target.shape[0]
            metrics['loss'] += loss.item() * batch_size
            meter.update(target, output)
    ...
    metrics['loss'] /= dataset_length
    metrics.update(meter.get_metrics())
    return metrics, best_imgs, worst_imgs
...
def main():
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        val_metrics, best_images, worst_images = val(model, val_loader, loss_f, metrics, device)
        write_metrics(epoch, val_metrics, val_writer)
        write_images(best_images, worst_images, val_writer)
